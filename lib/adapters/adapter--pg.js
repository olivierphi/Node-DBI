...
Adapter.prototype.removeBackticksFromTableNames = function( sql ) 
{
  ...
  sql = sql.replace(/from\s*`([^`]*)`/i, 'FROM $1');
  sql = sql.replace(/^\s*insert\s*into\s*`([^`]*)`/i, 'INSERT INTO $1');
  sql = sql.replace(/^\s*update\s*([^`]*)`/i, 'UPDATE $1');
  sql = sql.replace(/^\s*drop\s*table\s*`([^`]*)`/i, 'DROP TABLE $1');
  return sql;
};
...
{
  // TODO add config option indicating whether you wrote standard SQL
